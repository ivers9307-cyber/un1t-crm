  ...
  return (
    ...
    >
        ...
        options={{
          ...
          headerLeft: () => (
            <Pressable onPress={() => router.back()} hitSlop={10}>
              <Text className="text-base text-un1t-white">Cancel</Text>
            </Pressable>
          ),
          headerRight: () => (
            <Pressable onPress={submit} disabled={submitting} hitSlop={10}>
              {submitting ? (
                ...
                <Text className="text-base font-semibold text-un1t-white">Submit</Text>
              )}
            </Pressable>
          ),
        }}
      ...
      <ScrollView contentContainerClassName="p-4">
        ...
        <Text className="text-xs uppercase tracking-wider text-un1t-light px-2 mb-2">Type</Text>
        <View className="flex-row bg-un1t-dark border border-un1t-gray rounded-xl p-1 mb-5">
          {TYPES.map(t => (
            <Pressable
              key={t.value}
              onPress={() => setType(t.value)}
              className={`flex-1 py-2 rounded-lg ${type === t.value ? 'bg-un1t-white' : ''}`}
            >
              <Text className={`text-center text-sm ${type === t.value ? 'text-un1t-black font-semibold' : 'text-un1t-light'}`}>
                {t.label}
              </Text>
            </Pressable>
          ))}
        ...
        <Text className="text-xs uppercase tracking-wider text-un1t-light px-2 mb-2">Dates</Text>
        <View className="bg-un1t-dark border border-un1t-gray rounded-xl mb-5">
        ...
        <Text className="text-xs uppercase tracking-wider text-un1t-light px-2 mb-2">Reason (optional)</Text>
        <View className="bg-un1t-dark border border-un1t-gray rounded-xl mb-5">
          <TextInput
            value={reason}
            ...
            className="px-4 py-3 text-base text-un1t-white min-h-[100px]"
            textAlignVertical="top"
          />
        </View>

        <Text className="text-xs text-un1t-light px-2 mt-2">
...
function DateRow({ label, value, onChange, min, isLast }) {
  ...
  return (
    <View className={`flex-row items-center px-4 py-3 ${!isLast ? 'border-b border-un1t-gray' : ''}`}>
      <Text className="text-base text-un1t-white w-12">{label}</Text>
      <Text className="flex-1 text-base text-un1t-light">{pretty}</Text>
