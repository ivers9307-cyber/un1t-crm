...
import { freshVerdict, verdictFromResult, REGIONS_MAX_AGE_MS } from './physical-snapshot'
  ...
  const inFlight = (async () => {
    try {
      ...
      if (res?.success) {
        ...
        regionsCache = { at: Date.now(), regions }
        return regions
      }
 ...
 *   2. LAST-KNOWN — the OS's cached fix and the one we persisted at the end
 *      of the previous launch, whichever is fresher (pickFresherLastKnown).
 *   3. getCurrentPositionAsync, bounded by POSITION_TIMEOUT_MS.
 *
 * The ORDER of 2 and 3 is the reversal this ticket is about. Acquisition
 * used to come first, with lastKnown only as a fallback on failure — and
 * indoors, at the studio, in a steel-framed gym, that acquisition is exactly
 * where a cold start spends its seconds, behind a bare spinner, to learn what
 * the OS already knew. Nothing is loosened by preferring it: pickPosition's
 * 5-minute gate bounds how stale an accepted fix can be either way, and this
 * is the SAME data the old fallback already trusted — it is only trusted
 * sooner. A rejected last-known still falls through to a real acquisition,
 * which is the only case that now costs what every case used to.
 *
 * `forceFresh` skips tiers 1 and 2 entirely, for the two explicit "I may have
 * MOVED" signals — pull-to-refresh and a foreground after time away. Those
 * mean "re-read the radio", and a last-known fix is precisely the thing that
 * cannot answer them.
 */
async function readPosition({ persisted, forceFresh }) {
  if (!forceFresh) {
    ...
    if (lastKnown) {
      positionCache = { at: Date.now(), position: lastKnown }
      return lastKnown
    }
  }
...
async function resolveOnce(locations, { forceFresh = false } = {}) {
  ...
  // launch (writePhysicalSnapshot swallows its own failures — a >2 KB or
  // unwritable keychain costs a head start, never a screen). Written on
  // every outcome, not just at_studio: an offsite/unknown answer must CLEAR
  // the stored verdict, or the next launch paints tiles for a studio this
  // resolve just proved we are not in.
  // regionsAt is the region cache's OWN stamp, not now: an offline launch
  // re-persists the very regions it just read from disk, and stamping those
  // as fresh would keep a stale map of the studios alive past its 24h.
  writePhysicalSnapshot({ regions, regionsAt: regionsCache.at, position, result: next, nowMs: Date.now() })
  return next
}
