...
- **`race_registrations`** (mig 082, extended in 083) — one row per (race_event, team) with UNIQUE constraint preventing double-registration. Carries `status, race_started_at, race_finished_at, registered_at`. Replaces the bookings.race_started/finished_at pattern from mig 081. **`wave_id`** added in mig 083 — FK to race_waves, nullable at the schema level so race-event delete can cascade-set-null but required by app code at signup time.
- **`race_waves`** (mig 083) — one row per start-time slot in a race. Carries `start_time, capacity, label, display_order`. UNIQUE `(race_event_id, start_time)` so two waves can't share a start time. Replaces the deprecated `race_events.start_time` and `race_events.capacity` (kept on disk per the deprecated-columns convention; cleanup migration drops them later).
...
**Public/operator capacity split.** The public race API (`/api/public/races/[slug]`) deliberately strips raw capacity numbers from its response — neither race-level capacity nor per-wave `remaining_capacity` are ever exposed to public callers. Each wave object only carries `is_full: boolean`. The widget renders "Full" next to a wave card when `is_full`, nothing otherwise (clickability implies availability). Operator surfaces (`/races` index showing "X / Y teams", `<RaceEventForm>` with numeric capacity inputs per wave) go through the auth-gated `/api/races` endpoints which DO return raw capacity. Customers see "is this wave bookable" — operators see the actual numbers.
...
- **Member vs non-member registration + validation + payment.** Significant follow-on, captured here in detail because it touches schema + signup flow + race setup + an external payment integration. Three pieces:
  - **Race setup** (`<RaceEventForm>`): new fields on `race_events` for `member_policy` (`'members_only'` / `'members_and_non_members'` / `'open'`), `non_member_fee_eur` (NUMERIC, NULL when no fee), and possibly `member_validation_required` boolean. Operator decides per race what the policy is.
  - **Public signup** (`<RaceSignupWidget>`): each registrant (captain AND every team member) gets a "Are you a UN1T member?" toggle. For members: client validates their email against the contacts table — match requires `lead_status = 'member'` (or whatever set we consider "active member"; revisit `lead_status` taxonomy to confirm) at the same `location_id` as the race. Server re-validates on register so the client check is just UX. For non-members: capture is normal but the registration is flagged `is_member: false` on the team_members row (new column) so race-day staff can see the mix at a glance.
  - **Payment** (when non-members are registering AND `non_member_fee_eur` is set): same Revolut Embedded Checkout pattern as the car deposit feature (`src/lib/revolut.js` + `RevolutCheckout.embeddedCheckout`). Each non-member adds `non_member_fee_eur` to the captain's payable total. Registration goes into a `pending_payment` status until the Revolut webhook flips it to `confirmed`. Deposit-paid receipt SMS pattern from mig 078 can be reused for the "your race spot is paid + reserved" confirmation.
  - **New columns:** `team_members.is_member BOOLEAN`, `team_members.contact_match_id UUID NULLABLE` (the matched contact for members; useful for follow-up like "send all participating members an email"), `race_events.member_policy TEXT`, `race_events.non_member_fee_eur NUMERIC`, `race_registrations.payment_status TEXT` (`null` / `'pending'` / `'paid'` / `'failed'`), `race_registrations.payment_revolut_order_id UUID`.
  - **Edge cases worth thinking through up front:** what happens if a member's email doesn't match a contact at this location but matches at a sibling location in the same org (cross-location member)? What if the captain is a member but a teammate isn't — does the team still need to pay just for that one teammate? What's the refund policy for cancelled registrations (the existing `refundOrder()` helper from `src/lib/revolut.js` is wired and ready). Worth designing intentionally rather than discovering at implementation time.
