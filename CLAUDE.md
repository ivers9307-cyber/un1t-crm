...
> **Lint divergence — `next lint` ≠ bare `eslint .`.** The repo's `npm run lint` is `eslint .`, which does NOT fire the Next.js `@next/next/no-html-link-for-pages` rule — but CI's lint step does (it resolves the `app/` dir). So an `<a href="/internal-page">` passes `npm run lint` locally then **fails CI**. For any UI change that adds an internal link, use `<Link>` from `next/link` and run `npx next lint` (not just `npm run lint`) before pushing. (Bit on PR #353.)
...
- App Store Connect record created (ID `6770890839`, app name **CF Studio** — the `REBRAND.1` record; the previous UN1T-CRM record `6766947870` was removed), distribution method set to **Custom App for Business or Education**, ABM org listed as recipient.
...
4. The build appears under App Store Connect → My Apps → CF Studio → TestFlight tab once Apple finishes processing.
...
- Main Store Listing filled in (icon 512×512, feature graphic 1024×500, 3+ phone screenshots at 1080×2160, full description). Ready-made CF Studio icon + feature graphic live at `store-assets/play-android/` (generated from `mobile/assets/icon.png` via `sips`); screenshots still need capturing from a device/emulator.
