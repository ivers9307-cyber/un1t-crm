...
**Web/mobile boundary**
- **Mobile CANNOT import `src/lib`** → `shared/` is the seam, and not everything is re-exported. A mobile import of a non-exported name resolves to `undefined` and only crashes at runtime — `npm run check:mobile-imports` guards it (and `mobile/**` triggers Web CI).
