...
| `sms.js` | SMS broadcast engine (mig 060). `buildSmsAudience(db, filter, locationId)` returns the contacts query with the standard send-eligibility gates (active sms_status + has phone + at this location) plus the user's audience filter applied via `applyAudienceFilter`. `sendBroadcast(broadcastId)` flips the broadcast to 'sending', loops `sendLocationSms` per recipient, writes per-recipient rows to `sms_broadcast_recipients` + activities timeline entries, finalises with sent/failed counts. Rate-limits to 25 sends per second. Synchronous from caller's perspective (mirrors `whatsapp.js#sendBroadcast`); long sends will need a queued worker as Phase 2.5. |
