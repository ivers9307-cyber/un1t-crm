...
**Sequencing — richer triggers, esp. membership-state changes (operator-reported 2026-06)**
- The visual sequence builder's trigger list is narrower than operators expect. Concrete gap: under the **segment** trigger the dropdown only lists *saved* segments (operator-built on the Contacts page via AudienceBuilder), so with no saved segments it's empty — and there's **no direct way to trigger on a contact attribute** like `glofox_membership_state` (active / paused / **locked** = payment arrears, mig 196). Those fields ARE segmentable (`AUDIENCE_FIELDS` in `src/lib/audience-filter.js`), but only via the saved-segment indirection.
- Two complementary fixes: **(a)** add a `membership_state_change` trigger type — mirror `pipeline_stage_change` (from/to value, fired from the Glofox member-sync path when `glofox_membership_state` changes). This is the targeted one operators ask for and pairs directly with the **win-back/dunning** roadmap item (membership → `locked` → auto-start a dunning sequence). **(b)** OR an **inline audience condition on the trigger** (pick field+op+value from `AUDIENCE_FIELDS` directly in the trigger config, no saved segment) — more general but needs a runner change-detection mechanism. Also: in the builder's segment trigger, when there are no saved segments, surface a "create one on Contacts" link (discoverability).
- The builder UI piece is small once the engine supports it: add the trigger type to `src/lib/sequences/graph/schema.js` `TRIGGER_TYPES` + the `/api/sequences` route enums + a per-type config block in `SequenceSettings.jsx`. The real work is the runner trigger (`src/lib/sequences/triggers.js`) + its fire-point in the Glofox sync.
