...
| 4 | Week summary panel below ScheduleCalendar: per-coach FTE utilisation bars, contractor euro spend (visible month) vs `monthly_contractor_budget_eur`, empty-block count, unassigned-block list. Read-only / advisory. | 071 | Yes — surfaces the numbers, no enforcement. |
| 5 | `rosters` table + draft/published state. "Publish" action on the calendar. Over-budget publish requires owner approval (recorded on roster row). Email notify owner when manager-published roster needs approval. | 072 | Last — depends on 4. |
| 6 (deferred) | Leave-aware FTE availability. Subtract approved leave hours from contracted_hours_per_week in the utilisation calc. Needs a leave data source first. | — | Deferred. |
