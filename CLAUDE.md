...
- **A push to `main` touching a bundle path PUBLISHES AN OTA to production phones** (`eas-update.yml`, at 10%, with a 48h ramp-or-rollback obligation — `mobile/docs/ota-rollout.md`). The trigger is an **allowlist**, so adding a new directory under `mobile/` means deciding whether it ships: register it in the trigger *or* in `NON_BUNDLE` (`scripts/check-ota-trigger-paths.mjs`). `check:ota-paths` forces the choice. Never "fix" an over-triggering path by adding a `!` negation — that is the denylist this replaced.
