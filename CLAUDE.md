...
2. **Access matrix** — `<AdminAccessMatrix>` read-only grid: rows = staff (filterable by name/email), columns = locations grouped by organization, cells = role label or `—`. A "Master" badge column on the far right flags platform admins. Each row links to `/settings/staff/[id]` for editing — inline editing on the matrix itself is the v2 follow-up, deferred for safety (the matrix is the canonical "who has what" view; the per-staff editor is the canonical "change what they have" tool, and reusing the existing `StaffForm` wizard avoids re-implementing the per-location role/permissions/UniFi state in two places).

**Linking:** the entry point lives in the Master tools section on `/settings`. Look for the "Platform admin" button next to "View as user". Sidebar entry can come later if the page becomes a daily driver.
...
- Master admin matrix v2 — make the access matrix at `/admin/matrix` editable inline. Per-cell role dropdown (none / staff / head_coach / manager / owner), add-user-to-location action, master toggle (promote/demote). Today the matrix is read-only and links into `/settings/staff/[id]` for edits.
- "Add organization" UI on `/admin/matrix`. Today new organizations have to be inserted via SQL — fine while the platform has two and Richard owns both. When onboarding the third tenant becomes a real flow, this needs a button + form (name, slug, initial owner email/full_name) and probably an org-onboarding wizard.
