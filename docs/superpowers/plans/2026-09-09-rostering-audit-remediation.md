...
- **Task 3.1 — remove coach self-DELETE.** `src/app/api/schedule/assignments/[id]/route.js` DELETE: drop the `isSelf` allowance; non-managers get 403 with `error: 'Ask for a swap to drop this shift'`. Test file `assignments/[id]/route.test.js` (new): staff DELETE own → 403; manager at location → 200; manager elsewhere → 404. Web: `ScheduleCalendar.jsx:1804` `canEdit` splits into `canAdjust = isManager || a.profile_id === user.id` (Adjust button) and `canRemove = isManager` (X button). Mobile: `schedule.jsx` — find the self-remove affordance (grep `removeAssignment`); it is only in ManageMode (manager) so no change; confirm and note in the PR.
- **Task 3.2 — self-adjust is logged and pushes managers.** In the PUT handler, when `!wasManagerEdit && overrideChanged`: call `logRosterChange(db, { isPublished: assignment.shift_blocks?.rosters?.status === 'published', ..., action: 'time_changed', actorId: user.id, coachId: user.id, details: { self: true, start_time_override, end_time_override } })` and `sendPushToRolesAtLocationOnce(db, \`shift_self_adjusted:${data.id}:${newStart}-${newEnd}\`, locationId, MANAGER_ROLES, { title: 'Shift hours adjusted', body: \`${user.full_name} changed ${tplName} ${dateLabel} to ${newStart}–${newEnd}\`, category: 'schedule', data: { type: 'shift_self_adjusted', assignment_id: data.id } })`. Tests: self PUT logs + pushes; manager PUT pushes the coach (existing behaviour) and does not push managers.
- **Task 3.3 — change-log also on draft rosters for self edits.** `logRosterChange` currently no-ops when `!isPublished`; for self edits pass `isPublished: true` unconditionally (the audit trail matters regardless), and add a `details.roster_status` field. Test in `roster-change-log.test.js`.
- **Task 3.4 — gate + PR.** OTA-safe (JS only).
