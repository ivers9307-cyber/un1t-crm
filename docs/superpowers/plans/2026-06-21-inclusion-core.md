...
- [ ] **Add** `export async function createParticipationSession(db, { contactId, locationId, glofoxEventId, className, startedAtIso, endedAtIso, participationPoints, maxHr })` — inserts a `heart_rate_sessions` row: `source:'participation'`, `contact_id`, `location_id`, `started_at: startedAtIso`, `ended_at: endedAtIso`, `effort_points: participationPoints`, `zones_seconds: {}`, `max_hr_used: maxHr`, `glofox_event_id`, `class_name`, `class_link_source: 'booked'`, `device_identifier: null`. Returns the new row id. (Mirror the insert shape at `bridge-samples.js:337`.) **NOTE:** `class_link_source` has a CHECK constraint allowing only `'booked'`/`'presence'` — use `'booked'` (a participation credit comes from an attended *booking*); do NOT use `'attended'` (it would violate the constraint).
