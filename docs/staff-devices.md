...
`parseVersion` tolerates the junk that is actually in the column (leading `v`, missing minor/patch, `-beta`/`+build` suffixes) and compares **numerically**, so `2.10.0 > 2.9.0`. It rejects any segment above 9999 (the mobile register endpoint validates the same shape, so the two can't disagree about what is storable). Be clear about what that cap buys: `app_version` is client-reported and the highest value in the fleet becomes the bar everyone else is measured against, so the cap bounds the **magnitude** of a poisoned value — it does **not** remove the capability. Any authenticated staff session can still POST `'9999.0.0'` and mark the fleet outdated. Real defence would be an operator-set target or trusting only store-published builds; neither is built.
...
`mobile/components/LocationGate.jsx`'s `check` runs on mount and on every foreground. It maps the two OS reads to one value and calls `reportDeviceState()` (`mobile/lib/push-register.js`) **only when the value changed** since the last report (held in a ref). It is fire-and-forget inside its own `try/catch`: reporting must never block or break the gate. The ref only advances when the server confirmed the write (`res.result.success`) — `api()` never throws and never sets `skipped`, so anything less is retried on the next foreground.

> 🔴 **Never report mid-impersonation, and the guard lives inside `reportDeviceState`.** `api()` attaches `x-impersonate-target` straight from SecureStore, so a report sent while a master is viewing-as would resolve `getCurrentUser()` to the **target** and the upsert (conflict target `expo_push_token`) would **re-point the row's `user_id`** — handing the master's phone every subsequent push meant for the target (WhatsApp threads, lead alerts: customer PII) and cutting the master off from their own until next sign-in. The caller's React `impersonatingFrom` is **not** sufficient: it is populated by a fire-and-forget `/api/mobile/me` refresh that lands *after* `setSession`, so during cold start it is still null while SecureStore already says "impersonating" — exactly the window the gate's first `check()` runs in. `reportDeviceState` therefore reads `readImpersonate()` from SecureStore itself, mirroring `syncGeofences` in `mobile/lib/geofence.js`, which guards the same way for the same reason. The `LocationGate` check remains as a cheap early-out only.
...
| `/settings/staff` list | **At a glance.** A Device column (`2.1.0 · 2d`, an amber `Outdated` chip, the current device's permission chip, or a neutral `No app`) and a `Needs update` filter covering `outdated` + `no_device`. |
...
3. **One nudge per device per 24h**, held on `device_tokens.last_update_nudge_at` of the current device — and **claimed before sending**. Read-then-write would be a race: two concurrent POSTs (double-click, two tabs, a retry) would both read "not nudged yet" and both send. Instead the conditional UPDATE *is* the throttle — filtered to `last_update_nudge_at IS NULL OR < now-24h`, `.select('id')` — and **the rows it returns are the recipients**. Exactly one caller can win a row. If the send then delivers nothing, the claim is **released** back to each row's previous value (not blanket-nulled), so an Expo outage can't lock the operator out for a day. A claim that itself errors returns 500 rather than sending unthrottled.
