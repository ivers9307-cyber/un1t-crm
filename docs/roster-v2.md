...
4. **Writers** — `POST /api/schedule/shifts`, `/[id]` PUT/DELETE, copy-week, copy-month, plus the assistant `create_shift`. Today these write `shifts` and rely on the mig 069 reverse trigger to populate blocks/assignments; they need to write the new model directly.
