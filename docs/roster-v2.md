...
6. **Phase 6 — teardown (pending).** `public.shifts` now has **no reader**. Remaining before the table can be dropped:
   - Remove the 3 dead publish-path **writes** that still flip `shifts.published` (nothing reads it): `rosters/route.js`, `rosters/[id]/approve/route.js`, `shifts/publish/route.js`.
   - Drop the mig 068 forward trigger, the mig 069 reverse trigger, the block-cleanup trigger, and the `public.shifts` table itself.
   - Do this after a grace period to confirm nothing external (n8n confirmed unused) reads the table.
