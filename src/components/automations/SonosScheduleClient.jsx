...
function SonosScheduleInner({ locationName }) {
  ...
  const canEditSchedules = Boolean(household?.connected && household?.reachable)
  ...
  useEffect(() => {
    if (canEditSchedules) loadSchedules()
  }, [canEditSchedules, loadSchedules])

  // Mirrors the per-minute reconcile cron so last_applied/last_state on
  // screen don't need a manual refresh to catch up.
  useEffect(() => {
    if (!canEditSchedules) return undefined
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && !loadError && canEditSchedules && (
        <SchedulesSection
...
function SchedulesSection({ players, favorites, schedules, schedLoading, schedError, onReload, onAdd, adding, addError }) {
  ...
  return (
    <div className="space-y-3">
      <div className="bg-un1t-surface border border-un1t-border rounded-lg p-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-xs text-emerald-700">
            ...
            {favorites.length} favourite{favorites.length === 1 ? '' : 's'} available ✓
          </p>
      ...
      {schedules.length === 0 && !schedLoading && (
        <div className="bg-un1t-surface border border-un1t-border rounded-lg p-4">
          <p className="text-sm text-un1t-subtle">No schedules yet — add one below to start playing music on a timer.</p>
        </div>
      )}

      {schedules.map((s) => (
        <ScheduleCard key={s.id} schedule={s} players={players} favorites={favorites} onReload={onReload} />
      ))}
...
function ScheduleCard({ schedule, players, favorites, onReload }) {
  ...
  return (
    <div className="bg-un1t-surface border border-un1t-border rounded-lg p-4">
      <div className="flex items-start justify-between gap-4">
        <label className="text-xs text-un1t-subtle flex-1 min-w-40">
          ...
          <input type="text" value={name} onChange={(e) => { setName(e.target.value); setSaved(false) }}
            placeholder="e.g. Weekday classes"
            className="mt-1 w-full rounded border border-un1t-border bg-un1t-bg px-2 py-1 text-sm font-medium text-un1t-text" />
        </label>
        <div className="flex items-center gap-2 shrink-0 pt-4">
          ...
          <button
            ...
            title={!enabled && !canEnable ? 'Pick at least one speaker and add a window to enable' : (enabled ? 'Turn off' : 'Turn on')}
      ...
      {runState?.error && <p className="mt-2 text-[11px] text-red-700">{runState.error}</p>}

      <div className="mt-2 text-[11px] text-un1t-subtle">
        {schedule.last_applied
          ? <>Last applied: {schedule.last_applied.action === 'open' ? 'started' : 'paused'} {fmtDublin(schedule.last_applied.at)}
              {schedule.last_state?.playback_state ? ` · Sonos reports ${schedule.last_state.playback_state}` : ''}</>
          : 'Not applied yet.'}
      </div>
