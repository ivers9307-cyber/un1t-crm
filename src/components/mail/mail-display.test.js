...
//   • needsReply() is the one predicate the surface keeps from the conversation
...
// composer is remounted whenever the operator switches conversations specifically
// so member A's half-written reply cannot end up addressed to member B. A
// draft store that did not key on the conversation id would reopen that leak by a
...
// as per conversation (Richard's call). S() builds a scope with stable defaults so
// the pre-existing per-conversation assertions keep their meaning unchanged.
...
describe('reply drafts — scoped persistence', () => {
  ...
  it('returns null for a conversation with nothing saved', () => {
    expect(readReplyDraft(S('conversation-1'))).toBeNull()
  })

  it('round-trips a write', () => {
    writeReplyDraft(S('conversation-1'), { text: 'Sorry for the delay', mode: 'reply' })
    expect(readReplyDraft(S('conversation-1'))).toEqual({ text: 'Sorry for the delay', mode: 'reply' })
  })

  it('keeps note mode distinct from reply mode', () => {
    writeReplyDraft(S('conversation-1'), { text: 'staff-only', mode: 'note' })
    expect(readReplyDraft(S('conversation-1'))).toEqual({ text: 'staff-only', mode: 'note' })
  })

  it('falls back to reply mode for anything it does not recognise', () => {
    writeReplyDraft(S('conversation-1'), { text: 'hi', mode: 'bogus' })
    expect(readReplyDraft(S('conversation-1')).mode).toBe('reply')
  })

  // 🔴 THE ISOLATION GUARANTEE. A shared key here is the same class of bug
  // TICKET-COMPOSER-LEAK.1 already fixed once, one layer up.
  it('never lets one conversation read another conversation’s draft', () => {
    writeReplyDraft(S('conversation-A'), { text: 'For A only', mode: 'reply' })
    expect(readReplyDraft(S('conversation-B'))).toBeNull()
    writeReplyDraft(S('conversation-B'), { text: 'For B only', mode: 'reply' })
    expect(readReplyDraft(S('conversation-A'))).toEqual({ text: 'For A only', mode: 'reply' })
    expect(readReplyDraft(S('conversation-B'))).toEqual({ text: 'For B only', mode: 'reply' })
  })

  it('writing empty text clears rather than storing a blank draft', () => {
    writeReplyDraft(S('conversation-1'), { text: 'something', mode: 'reply' })
    expect(readReplyDraft(S('conversation-1'))).not.toBeNull()
    writeReplyDraft(S('conversation-1'), { text: '', mode: 'reply' })
    expect(readReplyDraft(S('conversation-1'))).toBeNull()
  })
  ...
  it('clearReplyDraft removes a stored draft directly', () => {
    writeReplyDraft(S('conversation-1'), { text: 'discard me', mode: 'reply' })
    clearReplyDraft(S('conversation-1'))
    expect(readReplyDraft(S('conversation-1'))).toBeNull()
  })
  ...
  it('caps stored draft length so one runaway paste cannot grow storage unboundedly', () => {
    ...
    writeReplyDraft(S('conversation-1'), { text: huge, mode: 'reply' })
    expect(readReplyDraft(S('conversation-1')).text.length).toBe(REPLY_DRAFT_MAX_LENGTH)
  })

  it('does nothing for a falsy conversation id, in every direction', () => {
  ...
  it('treats corrupt stored JSON as no draft rather than throwing', () => {
    window.localStorage.setItem('un1t.email.reply-draft.conversation-1', '{not json')
    expect(readReplyDraft(S('conversation-1'))).toBeNull()
  })
  ...
  it('survives storage being unavailable, in every direction', () => {
    ...
    expect(readReplyDraft(S('conversation-1'))).toBeNull()
    expect(() => writeReplyDraft(S('conversation-1'), { text: 'x', mode: 'reply' })).not.toThrow()
    expect(() => clearReplyDraft(S('conversation-1'))).not.toThrow()
...
describe('reply drafts — user and mailbox scoping (MAIL-DRAFTSCOPE.1)', () => {
  ...
  // conversation, different user → different key → no bleed.
  it('never lets one USER read another user\'s draft on the same conversation', () => {
    writeReplyDraft(S('conversation-1', 'staff-a'), { text: 'A\'s words', mode: 'reply' })
    expect(readReplyDraft(S('conversation-1', 'staff-b'))).toBeNull()
    expect(readReplyDraft(S('conversation-1', 'staff-a'))).toEqual({ text: 'A\'s words', mode: 'reply' })
  })

  it('scopes by email account: same conversation id under two mailboxes are two drafts', () => {
    writeReplyDraft(S('conversation-1', 'user-1', 'mb-accounts'), { text: 'for accounts@', mode: 'reply' })
    expect(readReplyDraft(S('conversation-1', 'user-1', 'mb-sales'))).toBeNull()
  })
  ...
  it('refuses to persist anything without a user id', () => {
    writeReplyDraft({ userId: null, mailboxId: 'mb-1', conversationId: 'conversation-1' }, { text: 'orphaned', mode: 'reply' })
    expect(window.localStorage.length).toBe(0)
    expect(readReplyDraft({ userId: null, mailboxId: 'mb-1', conversationId: 'conversation-1' })).toBeNull()
  })

  it('an orphan conversation (no mailbox) still persists, per user, under the none sentinel', () => {
    writeReplyDraft(S('conversation-1', 'user-1', null), { text: 'orphan draft', mode: 'reply' })
    expect(readReplyDraft(S('conversation-1', 'user-1', null))).toEqual({ text: 'orphan draft', mode: 'reply' })
    expect(readReplyDraft(S('conversation-1', 'user-1', 'mb-1'))).toBeNull()
    const key = Object.keys(window.localStorage).find(k => k.includes('.none.'))
    expect(key).toBeTruthy()
  })

  // Keys written by the pre-scope release (bare conversation id) are never read by
  ...
  it('legacy single-segment keys are still pruned by prefix', () => {
    window.localStorage.setItem('un1t.email.reply-draft.old-conversation',
      JSON.stringify({ text: 'stale', mode: 'reply', savedAt: Date.now() - (15 * 24 * 60 * 60 * 1000) }))
    writeReplyDraft(S('conversation-new'), { text: 'fresh', mode: 'reply' })
    expect(window.localStorage.getItem('un1t.email.reply-draft.old-conversation')).toBeNull()
  })
})
...
describe('reply drafts — eviction', () => {
  ...
  it('treats an entry past the TTL as absent, and clears it on read', () => {
    ...
    window.localStorage.setItem(
      `${REPLY_DRAFT_PREFIX}user-1.mb-1.conversation-old`,
      JSON.stringify({ text: 'ancient', mode: 'reply', savedAt: stale })
    )
    expect(readReplyDraft(S('conversation-old'))).toBeNull()
    // Cleared on the way out, not just hidden — a dead key must not sit
    // there for the next prune to rediscover.
    expect(window.localStorage.getItem(`${REPLY_DRAFT_PREFIX}user-1.mb-1.conversation-old`)).toBeNull()
  })

  it('keeps an entry comfortably inside the TTL', () => {
    const fresh = Date.now() - 1000
    window.localStorage.setItem(
      `${REPLY_DRAFT_PREFIX}user-1.mb-1.conversation-fresh`,
      JSON.stringify({ text: 'still good', mode: 'reply', savedAt: fresh })
    )
    expect(readReplyDraft(S('conversation-fresh'))).toEqual({ text: 'still good', mode: 'reply' })
  })
  ...
  it('treats a draft with no readable savedAt at all as expired', () => {
    ...
    // assume it is fine.
    window.localStorage.setItem(
      `${REPLY_DRAFT_PREFIX}user-1.mb-1.conversation-untimed`,
      JSON.stringify({ text: 'no timestamp', mode: 'reply' })
    )
    expect(readReplyDraft(S('conversation-untimed'))).toBeNull()
  })
  ...
  it('a write past the max-entry count evicts the oldest survivor, by savedAt', () => {
    ...
    for (let i = 0; i < REPLY_DRAFT_MAX_ENTRIES; i++) {
      window.localStorage.setItem(
        `${REPLY_DRAFT_PREFIX}user-1.mb-1.conversation-seed-${i}`,
    ...
    writeReplyDraft(S('conversation-new'), { text: 'the newest one', mode: 'reply' })

    // The very oldest seed (index 0) is gone…
    expect(readReplyDraft(S('conversation-seed-0'))).toBeNull()
    // …the newest seed survived…
    expect(readReplyDraft(S(`conversation-seed-${REPLY_DRAFT_MAX_ENTRIES - 1}`))).not.toBeNull()
    // …and so did the draft that triggered the prune in the first place.
    expect(readReplyDraft(S('conversation-new'))).toEqual({ text: 'the newest one', mode: 'reply' })
  })

  it('does not prune anything while under the max-entry count', () => {
    writeReplyDraft(S('conversation-a'), { text: 'a', mode: 'reply' })
    writeReplyDraft(S('conversation-b'), { text: 'b', mode: 'reply' })
    expect(readReplyDraft(S('conversation-a'))).not.toBeNull()
    expect(readReplyDraft(S('conversation-b'))).not.toBeNull()
  })
  ...
  it('a prune never touches a key outside its own prefix', () => {
    ...
    writeReplyDraft(S('conversation-new'), { text: 'triggers a prune', mode: 'reply' })
...
describe('clearAllReplyDrafts', () => {
  ...
  it('removes every draft this store holds, and reports how many', () => {
    writeReplyDraft(S('conversation-a'), { text: 'a', mode: 'reply' })
    writeReplyDraft(S('conversation-b'), { text: 'b', mode: 'reply' })
    writeReplyDraft(S('conversation-c'), { text: 'c', mode: 'note' })
    expect(clearAllReplyDrafts()).toBe(3)
    expect(readReplyDraft(S('conversation-a'))).toBeNull()
    expect(readReplyDraft(S('conversation-b'))).toBeNull()
    expect(readReplyDraft(S('conversation-c'))).toBeNull()
  })
