...
export function buildExpansionPrompt({ week, slot = 1, dial = 'mixed', locationLabel = 'UN1T', charter, houseStyle, styleExamples, autoTuneSignal = null, arcPlan, sessionsPerWeek, prevWeekSummary } = {}) {
  ...
  const system = [
    ...
    '- board.stations[].target: ONE clear class target for the station, up to about 20 characters - a distance, pace, load, reps, or calories, e.g. "400m", "2:30/500m", "9kg x 20", "60 cal", "2x16kg". The board shows a SINGLE target per station: do NOT split it into Performance and Elite here, and never a coaching sentence or nested object. The two-tier scaling (Performance vs Elite numbers) is described in full_session.main instead, never on the board.',
