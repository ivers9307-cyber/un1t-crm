...
import {
  ...
  isManagerTierRequester, approvedLeaveGuardAllows, annotateApprovedLeaveGuard, APPROVED_LEAVE_OWNER_ONLY_ERROR,
...
// needs the cancellation decider (an owner who is not the requester, or a master).
describe('isManagerTierRequester — the requester\'s role at a studio the request belongs to', () => {
  it('manager, head coach or owner at the filed-at studio: yes; staff: no', () => {
    for (const role of ['manager', 'head_coach', 'owner']) {
      expect(isManagerTierRequester([{ location_id: 'loc-1', role }], leave(), ['loc-1'])).toBe(true)
    }
    expect(isManagerTierRequester([{ location_id: 'loc-1', role: 'staff' }], leave(), ['loc-1'])).toBe(false)
    expect(isManagerTierRequester([], leave(), [])).toBe(false)
  })

  it('a manager role at their OTHER studio counts (leave covers the person, LEAVE.2)', () => {
    const rows = [{ location_id: 'loc-1', role: 'staff' }, { location_id: 'loc-2', role: 'manager' }]
    expect(isManagerTierRequester(rows, leave(), ['loc-1', 'loc-2'])).toBe(true)
  })

  it('a role at a studio the request does NOT belong to does not', () => {
    expect(isManagerTierRequester([{ location_id: 'loc-9', role: 'owner' }], leave(), ['loc-1'])).toBe(false)
  })
})
...
describe('approvedLeaveGuardAllows — who may move APPROVED leave out of force', () => {
  ...
  it('a manager-tier requester\'s approved leave: only an owner (not them) or a master', () => {
    for (const to of ['cancelled', 'rejected', 'pending']) {
      expect(approvedLeaveGuardAllows(OTHER_MANAGER, mgrLeave, to, ['loc-1'], true)).toBe(false)
      expect(approvedLeaveGuardAllows(person('hc', { 'loc-1': 'head_coach' }), mgrLeave, to, ['loc-1'], true)).toBe(false)
      expect(approvedLeaveGuardAllows(OWNER, mgrLeave, to, ['loc-1'], true)).toBe(true)
      expect(approvedLeaveGuardAllows(ANOTHER_OWNER, mgrLeave, to, ['loc-1'], true)).toBe(true)
      expect(approvedLeaveGuardAllows(MASTER, mgrLeave, to, ['loc-1'], true)).toBe(true)
    }
  })

  it('staff leave, pending leave and a re-stamp to approved are not this rule\'s business', () => {
    expect(approvedLeaveGuardAllows(OTHER_MANAGER, mgrLeave, 'cancelled', ['loc-1'], false)).toBe(true)
    expect(approvedLeaveGuardAllows(OTHER_MANAGER, leave({ status: 'pending' }), 'rejected', ['loc-1'], true)).toBe(true)
    expect(approvedLeaveGuardAllows(OTHER_MANAGER, mgrLeave, 'approved', ['loc-1'], true)).toBe(true)
  })

  it('the requester\'s own leave is LEAVECANCEL.1\'s path, not this one', () => {
    expect(approvedLeaveGuardAllows(MANAGER, mgrLeave, 'cancelled', ['loc-1'], true)).toBe(true)
  })

  it('an unreadable requester role (null) is judged as manager-tier: authority only narrows', () => {
  ...
  it('an owner only at a studio the request does not belong to is refused', () => {
    expect(approvedLeaveGuardAllows(person('own-9', { 'loc-9': 'owner' }), mgrLeave, 'cancelled', ['loc-1'], true)).toBe(false)
  })

  it('the refusal says what to do instead, with no em-dash', () => {
    expect(APPROVED_LEAVE_OWNER_ONLY_ERROR).toMatch(/^Only an owner can cancel a manager's approved leave\./)
    expect(APPROVED_LEAVE_OWNER_ONLY_ERROR).toMatch(/request the cancellation, or ask an owner\.$/)
    expect(APPROVED_LEAVE_OWNER_ONLY_ERROR).not.toMatch(/\u2014/)
  })
})
...
describe('annotateApprovedLeaveGuard — the list\'s per-row flag', () => {
  it('locked for a manager looking at a fellow manager\'s approved leave; not for an owner, and not on staff leave', () => {
    expect(annotateApprovedLeaveGuard(leave(), OTHER_MANAGER, ['loc-1'], true)).toEqual({ approved_locked_to_owner: true })
    expect(annotateApprovedLeaveGuard(leave(), OWNER, ['loc-1'], true)).toEqual({ approved_locked_to_owner: false })
    expect(annotateApprovedLeaveGuard(leave(), OTHER_MANAGER, ['loc-1'], false)).toEqual({ approved_locked_to_owner: false })
    expect(annotateApprovedLeaveGuard(leave({ status: 'pending' }), OTHER_MANAGER, ['loc-1'], true)).toEqual({ approved_locked_to_owner: false })
    expect(annotateApprovedLeaveGuard(leave(), MANAGER, ['loc-1'], true)).toEqual({ approved_locked_to_owner: false })
  })
})
