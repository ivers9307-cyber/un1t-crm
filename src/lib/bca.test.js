...
describe('getBcaConfig', () => {
  ...
  it('repairs corrupted documents array (wrong length)', () => {
    const c = getBcaConfig({
      features: { bca_submit: true },
      bca_config: { documents: [{ slug: 'doc_01', label: 'X' }] },  // only 1 entry
    })
    expect(c.documents).toHaveLength(10)
    expect(c.documents[0].slug).toBe('doc_01')  // back to default
  })
...
describe('validateBcaConfig', () => {
  ...
  it('rejects wrong document count', () => {
    const docs = Array.from({ length: 9 }, (_, i) => ({ slug: `doc_${String(i+1).padStart(2,'0')}`, label: 'x' }))
    const r = validateBcaConfig(valid({ documents: docs }))
    expect(r.ok).toBe(false)
    expect(r.errors.documents).toMatch(/exactly 10/)
  })
