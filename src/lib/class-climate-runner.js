// CLASS-CLIMATE.1 — runtime for the class-climate automation. Shared by
// the cron (/api/cron/class-climate) and the operator "Run check now"
// button (/api/automations/class_climate/run-now).
//
// For each location with the automation enabled, turn the configured AC
// device(s) ON for any class whose pre-class window is open and which
// hasn't been actioned yet — by writing a system ac_sessions row
// (started_by NULL) with auto_off_at = class_end + offset_off. The
// existing ac-auto-off cron performs the OFF; the external-rule cron sees
 ...
 */
export async function runClassClimateForLocation(db, automationRow, { nowMs = Date.now(), dryRun = false } = {}) {
  ...
  // Occurrences in a tight window (now-1h .. now+6h covers the pre-class
  // lead and a class that's already running).
  ...
  const planned = planClassClimate({ occurrences: occurrences || [], config, nowMs })
 ...
 * ac_sessions row with a class-aligned auto_off_at).
 */
async function fireOn(db, { locationId, deviceId, occurrence, config, nowMs }) {
  ...
  // next tick once the prior session ends).
...
async function recordFire(db, { locationId, eventId, deviceId, status, detail }) {
  ...
  if (error) logWarn('class-climate', 'fire-log write failed', { eventId, deviceId, error: error.message })
}

/**
 * Run for every location that has class_climate enabled (the cron path),
 * or a single location (the run-now path when locationId is given).
 * @param {object} db
 * @param {{ nowMs?: number, dryRun?: boolean, locationId?: string|null }} opts
 */
export async function runClassClimate(db, { nowMs = Date.now(), dryRun = false, locationId = null } = {}) {
  let q = db
  ...
  for (const row of rows || []) {
    locations.push(await runClassClimateForLocation(db, row, { nowMs, dryRun }))
  }
