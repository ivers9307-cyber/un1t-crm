...
describe('listBounces', () => {
  it('GETs /bounces with count, offset, tag, dates and the server token, and returns the shape', async () => {
    fetch.mockResolvedValueOnce(json({ TotalCount: 1, Bounces: [{ ID: 1, Type: 'SoftBounce', MessageID: 'm1', BouncedAt: 'b1' }] }))
    const r = await listBounces({ tag: 'host-campaign', fromDate: '2026-07-23', toDate: '2026-09-06', count: 500, offset: 0 })
    expect(r).toEqual({ total: 1, bounces: [{ ID: 1, Type: 'SoftBounce', MessageID: 'm1', BouncedAt: 'b1' }], error: null })
    const [url, init] = fetch.mock.calls[0]
    expect(url).toBe('https://api.postmarkapp.com/bounces?count=500&offset=0&tag=host-campaign&fromdate=2026-07-23&todate=2026-09-06')
    expect(init.headers['X-Postmark-Server-Token']).toBe('t')
    expect(init.method).toBe('GET')
  })
  it('a non-2xx is returned as error, never thrown', async () => {
    fetch.mockResolvedValueOnce(json({ Message: 'nope' }, false, 401))
    expect(await listBounces({ tag: 'x' })).toEqual({ total: 0, bounces: [], error: 'nope' })
  })
  it('a thrown fetch is returned as error', async () => {
    fetch.mockRejectedValueOnce(new Error('net'))
    expect(await listBounces({ tag: 'x' })).toEqual({ total: 0, bounces: [], error: 'net' })
  })
  it('no token → error, no fetch', async () => {
    delete process.env.POSTMARK_API_KEY
    expect((await listBounces({ tag: 'x' })).error).toMatch(/token/i)
    expect(fetch).not.toHaveBeenCalled()
  })
})
