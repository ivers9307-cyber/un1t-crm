...
export default async function AccessHistoryPage() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <Link href="/" className="inline-flex items-center gap-1 text-xs text-un1t-subtle hover:text-un1t-text mb-3">
      ...
      {rows.length === 0 ? (
        <div className="bg-un1t-surface border border-un1t-border rounded-2xl p-10 text-center">
          <ShieldAlert size={32} className="mx-auto mb-3 text-un1t-subtle" />
        ...
        <div className="bg-un1t-surface border border-un1t-border rounded-2xl overflow-x-auto">
                ...
                <tr key={r.id}>
                  <td className="p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm text-un1t-text">{r.masterName}</div>
                        <div className="text-xs text-un1t-subtle">{r.masterEmail}</div>
                      </div>
                    </div>
                  </td>
                  <td className="p-3 text-xs text-un1t-subtle whitespace-nowrap">{new Date(r.startedAt).toLocaleString()}</td>
                  <td className="p-3 text-xs text-un1t-subtle whitespace-nowrap">
                    {r.endedAt
                      ? new Date(r.endedAt).toLocaleString()
                      : <span className="text-amber-400">Active</span>}
                  </td>
                  <td className="p-3 text-xs text-un1t-subtle truncate max-w-[260px]" title={r.reason || ''}>{r.reason || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p className="mt-4 text-[11px] text-un1t-muted">
