...
describe('DELETE /api/live/[locationId]/register-device (unregister)', () => {
  ...
  it('404 when the contact is not at this location (IDOR guard)', async () => {
    ...
    const res = await DELETE(delReq({ device_key: 'ant:1', contact_id: '00000000-0000-0000-0000-000000000001' }), props)
    expect(res.status).toBe(404)
  })
  ...
  it('200 deactivates the device for a contact at this location', async () => {
    ...
    const res = await DELETE(delReq({ device_key: 'ant:45075', contact_id: '00000000-0000-0000-0000-000000000001' }), props)
    const json = await res.json()
    expect(res.status).toBe(200)
