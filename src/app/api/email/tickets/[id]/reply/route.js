...
export async function POST(request, props) {
  ...
  try {
    result = await sendEmail({
      // EMAIL-CC.1 — all three come out of toPostmarkFields(), the single site
      // where a resolved set becomes wire values. `bcc` reaches Postmark's own
      // Bcc field and nothing else; `headers` below carries threading only.
      to: wire.to,
      cc: wire.cc,
      bcc: wire.bcc,
      subject,
