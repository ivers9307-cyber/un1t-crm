...
// POST /api/schedule/reports/generate — Generate a report on demand
export async function POST(request) {
  ...
  if (!report_type || !period_start || !period_end) {
    return NextResponse.json({ success: false, error: 'report_type, period_start, and period_end are required' }, { status: 400 })
  }
  ...
  switch (report_type) {
    case 'staff_hours': {
      ...
      // Calculate hours per staff per day
      const staffHours = {}
      let totalHours = 0
    ...
    case 'utilisation': {
      ...
      // Weeks in period
      const periodStart = new Date(period_start + 'T00:00:00')
      const periodEnd = new Date(period_end + 'T00:00:00')
  ...
  const { data: saved, error } = await db.from('generated_reports').insert({
    location_id: locId,
