// SETTINGS.2g — server gate for /settings/scoring. Bare 'use client'
// component with no server-side check, relying entirely on PUT
// /api/settings/scoring 403ing after the shell loaded (GET is open to any
// signed-in user, same shape as /api/settings/customer-agent). Mirrors the
// write path's MANAGER_ROLES check (the page's own header comment already
// said "Manager+ only"). Signed-out -> /login, non-holder -> /settings
// (settings-family convention, same as src/app/settings/usage/page.js).
