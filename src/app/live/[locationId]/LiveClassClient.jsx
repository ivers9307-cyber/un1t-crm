  ...
  return (
    <main className="mx-auto max-w-7xl p-4 sm:p-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-un1t-light">Live class</p>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/"
            className="text-sm font-medium text-un1t-light hover:text-un1t-white"
          >
            Done
          </Link>
        </div>
      ...
      {loading && data.sessions.length === 0 ? (
        <p className="mt-10 text-center text-sm text-un1t-light">Loading live class…</p>
      ) : (
        <>
          <SessionGrid
...
function SessionGrid({ sessions, onEndOne }) {
  if (sessions.length === 0) {
    return (
      <div className="mt-10 rounded-2xl border border-dashed border-un1t-gray bg-white p-10 text-center">
        <Heart size={28} className="mx-auto text-un1t-light" />
      ...
      <div className="relative">
        <div className="flex items-start justify-between">
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-un1t-light">{session.contactName}</p>
          </div>
          <button
            type="button"
            onClick={onEnd}
            className="rounded p-1 text-un1t-light hover:bg-un1t-gray"
        ...
        <div className="mt-3 flex items-baseline justify-between">
          <p className="text-3xl font-bold tabular-nums">
            {session.currentBpm ?? '—'}
            <span className="ml-1 text-xs font-medium text-un1t-light">bpm</span>
          </p>
          {zone && (
            <span
        ...
        <p className="mt-2 text-[11px] text-un1t-light">
          {stale ? (
...
function AvailableStrapsPanel({ straps, onStartPair }) {
  ...
  return (
    <section className="mt-8 rounded-2xl border border-un1t-gray bg-white p-5">
      <div className="flex items-center gap-2">
        <Plug size={16} className="text-un1t-light" />
        <h2 className="text-sm font-semibold">Available straps</h2>
        <span className="ml-auto text-xs text-un1t-light">
        ...
        {straps.map((s) => (
          ...
          >
            ...
            <div className="text-right">
              {s.lastBpm != null && (
                <p className="text-sm font-semibold tabular-nums">{s.lastBpm}<span className="ml-0.5 text-[10px] text-un1t-light">bpm</span></p>
              )}
              <button
                type="button"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl">
        ...
        <input
          ...
          className="mt-3 w-full rounded-md border border-un1t-gray bg-white px-3 py-2 text-sm"
        />

        <ul className="mt-2 max-h-60 overflow-auto rounded-md border border-un1t-gray">
          {filtered.length === 0 ? (
            <li className="p-3 text-center text-xs text-un1t-light">No matches</li>
          ) : (
            filtered.map((c) => (
              <li key={c.id}>
                <button
                  ...
                  className="block w-full px-3 py-2 text-left text-sm hover:bg-un1t-dark disabled:opacity-50"
                >
                  {c.name}
                </button>
              </li>
