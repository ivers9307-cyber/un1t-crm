// REPLACE.1b — behavioural test for migration 642, mirroring 640's.
//
// Boots PGlite, runs the REAL mig 053 (cron_heartbeats + cron_health), adds mig
// 315's last_outcome column, runs 642, and proves:
...
import { SHIFT_OFFER_SWEEP_HEARTBEAT } from '@/lib/cron-arm-health'
...
const MIG_642 = read('642_shift_offer_sweep_heartbeat.sql')
...
describe('mig 642 seeds the shift-offer arm row', () => {
  it('under the name the code stamps, on the */5 arm cadence, born healthy', async () => {
    await runSql(MIG_642)
    expect(SHIFT_OFFER_SWEEP_HEARTBEAT).toBe('shift-offer-sweep')
    expect(await row(SHIFT_OFFER_SWEEP_HEARTBEAT)).toMatchObject({ expected_interval_seconds: 300, grace_seconds: 900 })
    expect((await db.query(`SELECT is_stale FROM public.cron_health WHERE name = $1`, [SHIFT_OFFER_SWEEP_HEARTBEAT])).rows[0].is_stale).toBe(false)
    expect((await row(SHIFT_OFFER_SWEEP_HEARTBEAT)).notes).toMatch(/send-push-reminders/)
    expect((await row(SHIFT_OFFER_SWEEP_HEARTBEAT)).notes).toMatch(/offerSweepArmHealthy/)
  })

  it('says in its header that it is applied right AFTER the deploy', () => {
    expect(MIG_642).toMatch(/APPLY THIS MIGRATION RIGHT AFTER THE REPLACE\.1b CODE DEPLOYS/)
  })

  it('two missed ticks (15 min) never page; 20 min without a clean run does', async () => {
    await runSql(MIG_642)
    expect(await staleAfter(SHIFT_OFFER_SWEEP_HEARTBEAT, 15 * 60 + 30)).toBe(false)
    expect(await staleAfter(SHIFT_OFFER_SWEEP_HEARTBEAT, 19 * 60)).toBe(false)
    expect(await staleAfter(SHIFT_OFFER_SWEEP_HEARTBEAT, 21 * 60)).toBe(true)
  })
  ...
  it('touches no other row', async () => {
    ...
    const after = (await db.query(`SELECT name, last_ok_at, expected_interval_seconds, grace_seconds FROM public.cron_heartbeats WHERE name <> $1 ORDER BY name`,
      [SHIFT_OFFER_SWEEP_HEARTBEAT])).rows
    expect(after).toEqual(before)
  })
})

describe('replay re-arms (ON CONFLICT DO UPDATE, as migs 601/623/633/640)', () => {
  it('re-arms a stale row, keeps the schedule and the arm\'s last_outcome', async () => {
    await runSql(MIG_642)
    const outcome = { open: 2, claimed_owed: 0, sent: 1, none: 1, errors: 0 }
    await db.query(`UPDATE public.cron_heartbeats SET last_outcome = $2::jsonb WHERE name = $1`, [SHIFT_OFFER_SWEEP_HEARTBEAT, JSON.stringify(outcome)])
    expect(await staleAfter(SHIFT_OFFER_SWEEP_HEARTBEAT, 30 * 60)).toBe(true)
    await runSql(MIG_642)
    expect(await ageSeconds(SHIFT_OFFER_SWEEP_HEARTBEAT)).toBeLessThan(5)
    expect(await row(SHIFT_OFFER_SWEEP_HEARTBEAT)).toMatchObject({ expected_interval_seconds: 300, grace_seconds: 900, last_outcome: outcome })
    expect((await db.query(`SELECT count(*)::int AS n FROM public.cron_heartbeats WHERE name = $1`, [SHIFT_OFFER_SWEEP_HEARTBEAT])).rows[0].n).toBe(1)
  })

  it('rewrites a hand-tuned or pre-existing cadence back to the intended one', async () => {
    await db.query(`INSERT INTO public.cron_heartbeats (name, expected_interval_seconds, grace_seconds, notes) VALUES ($1, 60, 60, 'stray')`, [SHIFT_OFFER_SWEEP_HEARTBEAT])
    await runSql(MIG_642)
    expect(await row(SHIFT_OFFER_SWEEP_HEARTBEAT)).toMatchObject({ expected_interval_seconds: 300, grace_seconds: 900 })
    expect((await row(SHIFT_OFFER_SWEEP_HEARTBEAT)).notes).toMatch(/^REPLACE\.1b/)
  })
})
...
describe('the self-check reads the post-state', () => {
  it('a row that does not END UP on the intended schedule aborts the WHOLE file', async () => {
    await runSql(`
      ...
      BEGIN
        IF NEW.name = 'shift-offer-sweep' THEN NEW.grace_seconds := 60; END IF;
        RETURN NEW;
      END $$;
    ...
    `)
    await expect(runSql(MIG_642)).rejects.toThrow(/mig 642: cron_heartbeats row shift-offer-sweep .*300.*900/)
    expect(await row(SHIFT_OFFER_SWEEP_HEARTBEAT)).toBeUndefined()
  })
})
