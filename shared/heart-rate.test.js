...
// ── resolveScoringConfig: tier_window_months (tier decay, default-off) ──

describe('resolveScoringConfig tierWindowMonths', () => {
  it('defaults to null (no decay) when settings.scoring absent', () => {
    expect(resolveScoringConfig(undefined).tierWindowMonths).toBeNull()
    expect(resolveScoringConfig({}).tierWindowMonths).toBeNull()
    expect(resolveScoringConfig({ settings: {} }).tierWindowMonths).toBeNull()
    expect(resolveScoringConfig({ settings: { scoring: {} } }).tierWindowMonths).toBeNull()
  })
  it('reads a positive integer window', () => {
    expect(resolveScoringConfig({ settings: { scoring: { tier_window_months: 6 } } }).tierWindowMonths).toBe(6)
  })
  it('truncates a fractional window and rejects < 1 / non-numeric as null', () => {
    expect(resolveScoringConfig({ settings: { scoring: { tier_window_months: 6.9 } } }).tierWindowMonths).toBe(6)
    for (const bad of [0, -3, null, 'x', NaN]) {
      expect(resolveScoringConfig({ settings: { scoring: { tier_window_months: bad } } }).tierWindowMonths).toBeNull()
    }
  })
  it('still resolves zone/participation points unchanged', () => {
    const c = resolveScoringConfig({ settings: { scoring: { tier_window_months: 6 } } })
...
describe('ZONE_DEFS', () => {
  ...
  // Customer-facing copy rule (Richard): zone labels must be spelled out
  // in full ("Zone 4"), never the "Z4" shorthand. This `label` is the single
  // source of truth shown to members via zoneBreakdown → the UI.
  it('labels spell zones out in full — never the Z-shorthand', () => {
    expect(ZONE_DEFS.map((z) => z.label)).toEqual(['Zone 1', 'Zone 2', 'Zone 3', 'Zone 4', 'Zone 5'])
    for (const z of ZONE_DEFS) expect(z.label).not.toMatch(/^Z\d/)
  })
})
