...
// 208 - 0.7*age (clamped 140..220). Tanaka is more accurate than the
// classic 220-age above 40; the +/-10bpm CI on either formula makes the
// difference noise for zone allocation, so we don't expose a choice.

export const ZONE_DEFS = [
  { id: 1, label: 'Zone 1', name: 'Warm-up',  pctMin: 0,    pctMax: 0.60, color: '#9CA3AF', points: 1 },
  { id: 2, label: 'Zone 2', name: 'Easy',     pctMin: 0.60, pctMax: 0.70, color: '#3B82F6', points: 2 },
  { id: 3, label: 'Zone 3', name: 'Aerobic',  pctMin: 0.70, pctMax: 0.80, color: '#10B981', points: 3 },
  { id: 4, label: 'Zone 4', name: 'Threshold',pctMin: 0.80, pctMax: 0.90, color: '#F59E0B', points: 4 },
  { id: 5, label: 'Zone 5', name: 'Max',      pctMin: 0.90, pctMax: 1.10, color: '#EF4444', points: 5 },
]
 ...
 * @returns {{ zonePoints: { 1:number,2:number,3:number,4:number,5:number }, participationPoints: number, tierWindowMonths: number|null }}
 */
export function resolveScoringConfig(location) {
  ...
  // Tier decay (approved decision #4). Rolling-window length in months; a
  // positive integer enables decay, absent/null/invalid = null = NO DECAY
  // (cumulative tier, the default). Shared contract with un1t-crm operator config.
  const rawWindow = Math.trunc(Number(s?.tier_window_months))
  const tierWindowMonths = Number.isFinite(rawWindow) && rawWindow >= 1 ? rawWindow : null
  return { zonePoints, participationPoints: num(s?.participation_points, SCORING_DEFAULTS.participation_points), tierWindowMonths }
}
...
export function resolveMaxHr(contact, referenceDate = Date.now()) {
  ...
  if (age == null) {
    ...
    //
    // KNOWN BIAS: a flat 180 default is LOW for younger members and HIGH
    // for older ones. Since points now feed challenges/tiers, a member
    // with no dob gets a maxHr that inflates or deflates their zone
    // allocation vs a member whose dob (or Tanaka estimate) is known —
    // i.e. the default is gameable (leave dob blank to sit in a higher
    // zone for the same bpm and farm points). Do NOT change this value
    // in isolation: it shifts scoring estate-wide and would silently
    // re-rank every no-dob member's historical sessions.
    // Wave 4 product decision: require dob before scoring counts toward
    // challenges, or fall back to a per-location median maxHr.
    return 180
  }
